...
//     const { audioBase64, ...otherData } = eventData;
//     const audioBlob = base64ToBlob(audioBase64);
//     const audioUrl = URL.createObjectURL(audioBlob);

//     setTranslatedTranscriptions(prevTranslations => [
//       ...prevTranslations,
//       { ...otherData, audioUrl }
//     ]);
//   };
...
//   // useEffect(() => {
//   //   const baseUrl = "https://ivaanibackend.indikaai.com/";
...
//   // useEffect(() => {
//   //   let ws = new WebSocket('wss://ivaanibackend.indikaai.com/ws');
...
//   //     setTimeout(() => {
//   //       ws = new WebSocket('wss://ivaanibackend.indikaai.com/ws');
...
//     const baseURL = 'https://ivaanibackend.indikaai.com/';
//     try {
//       const response = await axios.get(`${baseURL}download_recording`, {
//         responseType: 'blob',
//       });
...
//       const link = document.createElement('a');
//       link.href = url;
//       link.setAttribute('download', 'recording.mp3'); // or any other extension
//       document.body.appendChild(link);
//       link.click();
//       document.body.removeChild(link);
//     } catch (error) {
...
//           </div>
//           <div className="flex border-gray-200 p-4 m-4 items-center justify-between" style={{ backgroundColor: "#F8F8F8" }}>
//             <div className="font-semi-bold" style={{ color: "black" }}>Download Audio Data</div>
//             <DownloadIcon onClick={downloadRecording} style={{ marginRight: 12, cursor: 'pointer' }} />
//           </div>
//         </Sider>
//         <Layout
//           style={{
...
//               </div>
//             </div>
//             <div ref={containerRef} className="flex flex-col border-gray-200 p-4 m-4 items-start" style={{ backgroundColor: "#F8F8F8", minHeight: '55vh', maxHeight: '55vh', overflowY: 'auto' }}>
...
//                         <source src={conversation.audioUrl} type="audio/wav" />
//                       </audio>
//                     )} */}
...
const Dashboard = () => {
  ...
  const handleTranscription = (transcriptionData) => {
    setMessages(prevMessages => [
      ...prevMessages,
      { ...transcriptionData, type: 'transcription' }
    ].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)));
  };

  const handleTranslatedText = (translatedData) => {
    setMessages(prevMessages => [
      ...prevMessages,
      { ...translatedData, type: 'response' }
    ].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)));
  };

  useEffect(() => {
    const baseUrl = "https://ivaani.roadvision.ai/";
    let localDevice = null;
    const setupTwilioDevice = async () => {
      try {
        const tokenResponse = await fetch(`${baseUrl}token`);
        ...
        const processor = new BackgroundAudioProcessor(null, handleTranscription, handleTranslatedText);
        await device.audio.addProcessor(processor);
  ...
  const downloadTranscriptions = () => {
    const combinedData = messages.map((message) => ({
      speaker: message.type === 'transcription' ? 'Caller' : 'Callee',
      transcription: message.transcription || message.response_text,
          ...
          <Content
            ...
            <div ref={containerRef} className="flex flex-col border-gray-200 p-4 m-4 items-start" style={{ backgroundColor: "#F8F8F8", minHeight: '55vh', maxHeight: '55vh', overflowY: 'auto' }}>
              {messages.map((conversation, index) => (
                <div
                  key={index}
                  className={`flex border-gray-200 p-4 m-4 ${conversation.type === 'transcription' ? 'justify-end' : 'justify-start'} ${conversation.type === 'transcription' ? 'ml-auto' : ''}`}
                  style={{ backgroundColor: conversation.type === 'transcription' ? "#E6E0FD" : "#FFFFFF" }}
                >
                  <div className="flex flex-col">
                    <div className={`flex items-center ${conversation.type === 'transcription' ? 'ml-auto' : ''}`}>
                      <img
                        src={conversation.type === 'transcription' ? callerlang : operatorlang}
                        alt={conversation.type === 'transcription' ? "Caller Language" : "Operator Language"}
                        style={{ marginRight: 12, width: 50, height: 50 }}
                      />
                      <div className="font-semi-bold">{conversation.type === 'transcription' ? "Caller" : "You"}</div>
                    </div>
                    <div className="font-bold">{conversation.transcription || conversation.response_text}</div>
                    <div className="font-semi-bold text-[#2200F3]">{conversation.translated_text || conversation.callee_translatedText}</div>
                    <div className="font-semi-bold">{conversation.caller || conversation.callee_caller}</div>
                    <div className="font-semi-bold">{new Date(conversation.timestamp || conversation.callee_timestamp).toLocaleString()}</div>
                  </div>
