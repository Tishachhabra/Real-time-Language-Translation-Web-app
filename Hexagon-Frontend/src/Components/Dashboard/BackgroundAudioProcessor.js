...
// //     if (!this.ws) {
// //       this.ws = new WebSocket("wss://ivaanibackend.indikaai.com/callee_transcription");
// //       this.ws.onopen = () => {
...
//     // Removed stream parameter as we are not using client's microphone stream
...
//       this.ws.onopen = () => {
//         console.log("WebSocket connection established for callee transcription");
//         this.transcribeAudio().then(transcription => {
//           console.log("Transcription process completed:", transcription);
//         }).catch(error => {
//           console.error("Transcription error:", error);
//         });
...
//     // Removed stream parameter and related code
...
export default class BackgroundAudioProcessor {
  ...
  async createProcessedStream() {
    if (!this.ws) {
      this.ws = new WebSocket("wss://ivaani.roadvision.ai/ws");
      this.ws.onopen = () => {
        console.log("WebSocket connection established for callee transcription");
        this.transcribeAudio().then(transcription => {
          console.log("Transcription process completed:", transcription);
      ...
      this.ws.onmessage = (event) => {
        ...
        if (data.type === 'transcription' && this.transcriptionCallback) {
        ...
        if (data.type === 'response' && this.translatedtextCallback) {
          const eventData = {
